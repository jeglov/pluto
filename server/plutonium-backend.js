 ...
 */
class Plutonium {
	static async post (req, res) { // Flagrant abuse of HTTP verbs
		const {logger} = global;
	...
	static _getSafeDirectoryPath (dirPath) {
		if (!dirPath) return {
			serverDirPath: path.join(global.paths.data),
			clientDirPath: "",
		};
		const cleanPath = path.normalize(this._getCleanDirPath(dirPath)).replace(/^(\.\.(\/|\\|$))+/, "");
		return {
			serverDirPath: path.join(global.paths.data, cleanPath),
			clientDirPath: cleanPath,
		}
	}
	...
	static async _pHandleArtBrowserCopyToLocal (req, res) {
		...
		try {
			...
			const outputPath = path.join(serverDirPath, filename);
			...
			res.send({path: path.join(clientDirPath, filename)});
		} catch (e) {
			logger.error(e);
	...
	static async _pHandleArtBrowserDownloadPack (req, res) {
		...
		try {
			...
			const dirPath = path.join(serverDirPath, dirName);
